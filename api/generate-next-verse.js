...
function transcriptWordCount(lesson) { return String(lesson?.transcript || '').trim().split(/\s+/).filter(Boolean).length; }
...
async function enforceExperienceGate(lesson, reference, apiKey, model) {
  let candidate = lesson; let evaluation; let readability;
